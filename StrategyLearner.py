...
class StrategyLearner(object):
    ...
    def addEvidence(self, symbol="IBM",
        ...
        for ind in self.indicators:
            self.div_dict[ind] = self._bin_divider(self.df[ind], N=self.bins, method='even')

        # without considering holdings, get partial states based on indicators all at once
        ind_states = self._get_state(self.df[self.indicators], self.div_dict)
        self.df['Ind_States'] = ind_states
        ...
        for iteration in range(0,it):
            ...
            holdings = 0
            start_state = self._full_state(self.df.loc[sd,'Ind_States'], holdings, num_of_ind=len(self.indicators))
        ...
        self.total_states = list(set(self.df.Ind_States.values))
    ...
    def testPolicy(self, symbol = "IBM",
        ...
        for ind in indicators:
            self.div_dict[ind] = self._bin_divider(self.tdf[ind], N=self.bins, method='even')

        # without considering holdings, get partial states based on indicators all at once
        ind_states = self._get_state(self.tdf[indicators], self.div_dict)
        self.tdf['Ind_States'] = ind_states
    ...
    @staticmethod
    def _bin_divider(df_series, N=10, method='quantile'):
        '''
    ...
    @staticmethod
    def _get_position(indicator, divider_list):
    ...
    def _get_state(self, ind_values, divider_dict):
        ...
        if isinstance(ind_values, pd.DataFrame):
            ...
            for indicator, i in zip(inds, range(len(divider_dict))):
                s[i] = self._get_position(ind_values[indicator], divider_dict[indicator])
            ...
            state = 0
            for indicator, i in zip(inds, range(len(divider_dict))):
                s[i] = self._get_position(pd.Series(ind_values[indicator]), divider_dict[indicator])
                state = state + s[i] * 10 ** i
        else:
    ...
    @staticmethod
    def _full_state(ind_states, holdings, num_of_ind=3):
